...
import {
  Crown, Shield, BarChart3, Megaphone, Palette,
  MessageSquare, Users, Settings, ArrowLeft, TrendingUp,
  Bell, Star, CheckCircle, XCircle, Clock
} from "lucide-react";
...
export default function Admin() {
  ...
  const [newUpdate, setNewUpdate] = useState({ title: "", description: "", version: "", type: "feature" as const });

  const createUpdate = trpc.updates.create.useMutation({
    onSuccess: () => {
      toast.success("تم إضافة التحديث بنجاح!");
      setNewUpdate({ title: "", description: "", version: "", type: "feature" });
    },
    ...
    return (
      <div className="min-h-screen bg-[#0a0a0a] flex items-center justify-center" dir="rtl">
        <div className="text-center max-w-md px-4">
          ...
          <p className="text-gray-400 mb-8">تحتاج لتسجيل الدخول للوصول لهذه الصفحة</p>
          <a href={getLoginUrl()} className="btn-gold px-8 py-4 rounded-xl font-black text-lg inline-block">
            تسجيل الدخول
          </a>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-[#0a0a0a] text-white" dir="rtl">
      ...
      <div className="pt-24 pb-16 max-w-7xl mx-auto px-4">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-500 mt-1">مرحباً {user.name}، لديك صلاحيات المشرف الكاملة</p>
        </div>

        {/* Stats */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          {[
            { label: "إجمالي الحملات", value: stats?.campaigns || 0, icon: Megaphone, color: "text-blue-400", bg: "bg-blue-900/20" },
            { label: "طلبات التصميم", value: stats?.designs || 0, icon: Palette, color: "text-purple-400", bg: "bg-purple-900/20" },
            { label: "آراء العملاء", value: stats?.feedback || 0, icon: MessageSquare, color: "text-yellow-400", bg: "bg-yellow-900/20" },
          ].map((stat, i) => (
            <motion.div
              key={i}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="bg-[#111] rounded-2xl p-6 border border-red-900/20">
            ...
            <div className="space-y-4">
              <div>
                <label className="text-gray-400 text-sm mb-2 block">نوع التحديث</label>
                <div className="grid grid-cols-2 gap-2">
                  {[
                    ...
                    <button
                      key={t.id}
                      onClick={() => setNewUpdate(prev => ({ ...prev, type: t.id as typeof newUpdate.type }))}
              ...
              <div>
                <label className="text-gray-400 text-sm mb-2 block">العنوان *</label>
                <input
                  type="text"
              ...
              <div>
                <label className="text-gray-400 text-sm mb-2 block">رقم الإصدار</label>
                <input
                  type="text"
                    ...
                    <div className="flex items-center justify-between mb-2">
                      ...
                      {fb.rating && (
                        <div className="flex gap-0.5">
                          {[1, 2, 3, 4, 5].map(s => (
                            <Star key={s} className={`w-3 h-3 ${s <= fb.rating! ? "text-yellow-500 fill-yellow-500" : "text-gray-700"}`} />
                          ))}
                        </div>
                      )}
                    </div>
        ...
        <div className="mt-8 grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { icon: Users, label: "إدارة المستخدمين", color: "bg-blue-900/20 text-blue-400" },
            { icon: Settings, label: "إعدادات المنصة", color: "bg-purple-900/20 text-purple-400" },
            { icon: TrendingUp, label: "تقارير الأداء", color: "bg-green-900/20 text-green-400" },
            { icon: Bell, label: "الإشعارات", color: "bg-yellow-900/20 text-yellow-400" },
          ].map((item, i) => (
            <button
              key={i}
      ...
      <footer className="bg-[#080808] border-t border-red-900/20 py-6">
        <div className="max-w-7xl mx-auto px-4 text-center">
          <p className="text-gray-600 text-sm">
            © {new Date().getFullYear()} REX-SHOP Admin Panel. جميع الحقوق محفوظة لـ REX-SHOP™
          </p>
        </div>
      </footer>
    </div>
